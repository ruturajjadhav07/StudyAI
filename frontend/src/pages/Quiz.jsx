...
export default function Quiz() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      {/* Top Bar */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4 sticky top-0 z-10 transition-colors">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="font-bold text-xl text-slate-800 dark:text-white">📝 Quiz</span>
          ...
          <button
            ...
            disabled={submitting}
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white font-semibold rounded-xl transition"
          >
      ...
      <div className="max-w-5xl mx-auto px-6 py-4 flex gap-6 items-start">
        {/* Question Area */}
        <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          ...
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 leading-relaxed mb-8 transition-colors">
            {q.questionText}
          </h2>
            ...
            {['A', 'B', 'C', 'D'].map(opt => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-slate-700 dark:text-slate-300 transition-colors">{q[`option${opt}`]}</span>
                  {selected && (
          ...
          <div className="flex justify-between">
            <button
              ...
              disabled={current === 0}
              className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-400 font-semibold text-sm disabled:opacity-30 hover:bg-slate-50 dark:hover:bg-slate-800 transition"
            >
            ...
            {current < questions.length - 1 ? (
              <button
                onClick={() => setCurrent(c => c + 1)}
                className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm rounded-xl transition"
              >
              ...
              <button
                onClick={handleSubmit}
                disabled={submitting}
                className="px-6 py-2.5 bg-green-500 hover:bg-green-600 text-white font-bold text-sm rounded-xl transition"
              >
                Finish Quiz 🎉
              </button>
            )}
        ...
        {/* Sidebar */}
        <div className="w-56 flex-shrink-0 bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 sticky top-24 transition-colors">
          ...
          <div className="space-y-2 text-xs border-t border-slate-100 dark:border-slate-800 pt-4 transition-colors">
            {[
              { color: 'bg-indigo-600', label: 'Current' },
              { color: 'bg-green-400', label: 'Answered' },
              { color: 'bg-amber-400', label: 'Flagged' },
              { color: 'bg-slate-200 dark:bg-slate-700', label: 'Not answered' },
            ].map(l => (

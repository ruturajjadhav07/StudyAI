...
import { SiTailwindcss, SiMongodb, SiGooglecloud, SiSpringboot } from "react-icons/si";
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 selection:bg-indigo-100 dark:selection:bg-indigo-900 overflow-x-hidden relative transition-colors duration-500 font-sans">
            ...
            <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl z-50 px-4 md:px-6 py-3 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-white/40 dark:border-slate-800/40 rounded-2xl shadow-xl flex items-center justify-between transition-all">
                <button onClick={scrollToTop} className="flex items-center gap-2 group cursor-pointer bg-transparent border-none outline-none">
                    ...
                    <span className="text-xl font-bold text-slate-800 dark:text-white tracking-tight">StudyAI</span>
                </button>
                ...
                <div className="flex items-center gap-3 md:gap-5">
                    ...
                    <div className="flex items-center gap-2">
                        <Link to="/login" className="text-sm font-bold text-slate-700 dark:text-slate-300 hover:text-indigo-600 transition px-2">
                            Login
                        </Link>
                        <Link to="/register" className="bg-slate-900 dark:bg-indigo-600 text-white px-4 md:px-6 py-2 md:py-2.5 rounded-xl text-xs md:text-sm font-bold hover:bg-indigo-600 hover:scale-105 transition-all shadow-lg whitespace-nowrap">
                            Get Started Free
                        </Link>
                    </div>
                </div>
            ...
            <header className="pt-48 pb-20 px-4 text-center">
                <div className="max-w-4xl mx-auto">
                    <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-bold uppercase tracking-widest mb-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <Link to="/register" className="w-full sm:w-auto px-10 py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg hover:shadow-2xl hover:shadow-indigo-200 transition-all flex items-center justify-center gap-2 group">
            ...
            <section id="features" className="py-24 px-4 bg-white/30 dark:bg-slate-900/30 backdrop-blur-sm">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className="text-4xl font-bold text-slate-900 dark:text-white tracking-tight">Capabilities</h2>
            ...
            <section id="how-it-works" className="py-24 px-6 max-w-6xl mx-auto">
                <h2 className="text-4xl font-bold text-center mb-16 text-slate-900 dark:text-white tracking-tight">How It Works</h2>
                <div className="grid md:grid-cols-3 gap-12">
                    {[
                        ...
                        <div key={i} className="relative bg-white/40 dark:bg-slate-800/40 p-10 rounded-[2.5rem] text-center border border-white dark:border-slate-700 shadow-md">
                            <div className="w-16 h-16 mx-auto flex items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white text-2xl font-black mb-6 shadow-lg">
                                {item.step}
                            </div>
            ...
            {/* TECH STACK SECTION */}
            <section id="tech-stack" className="py-24 px-4 bg-indigo-50/50 dark:bg-slate-950/50 border-y border-slate-100 dark:border-slate-800">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className="text-4xl font-bold text-slate-900 dark:text-white tracking-tight">The Tech Behind StudyAI</h2>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        {techStack.map((tech, i) => (
                            <div key={i} className="flex flex-col items-center p-6 rounded-[2rem] bg-white dark:bg-slate-900 border border-white dark:border-slate-800 shadow-sm hover:border-indigo-500/50 transition-all group">
                                <div className="text-3xl mb-4 group-hover:scale-110 transition-transform duration-300">
                                    {tech.icon}
                                </div>
                                <span className="font-bold text-slate-800 dark:text-white text-xs">{tech.name}</span>
                                <span className="text-[9px] uppercase tracking-widest text-slate-400 dark:text-slate-500 mt-1 font-bold">{tech.desc}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="max-w-6xl mx-auto bg-gradient-to-r from-indigo-600 to-purple-700 rounded-[3rem] p-12 md:p-20 text-center text-white relative overflow-hidden shadow-2xl">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-indigo-100 text-lg md:text-xl mb-10 max-w-2xl mx-auto font-medium opacity-90">
                            We believe education should be accessible. No hidden fees, no subscriptions—just pure AI-powered learning for everyone.
                        </p>
                        <Link to="/register" className="inline-block px-10 py-5 bg-white text-indigo-600 rounded-2xl font-extrabold text-xl hover:scale-105 transition-all shadow-xl">
                            Get Started Now
                        </Link>
                    </div>
                </div>
